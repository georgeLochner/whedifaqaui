...
#
# Launches a Claude CLI agent, waits for completion, extracts results.
# Monitoring of agent progress is the coordinator's responsibility via
# the central log. This script only enforces a hard max-runtime safety cap.
...
Monitoring:
    ...
    Use: tail -20 <central-log>
    To kill a stuck agent: kill \$(cat <output-dir>/agent.pid)

Exit Codes:
    0 - Success
    2 - Maximum runtime exceeded (safety limit)
...
# Cleanup function (kills agent, watchdog, and logger if interrupted; preserves all logs)
WATCHDOG_PID=""
cleanup() {
    local exit_code=$?
    ...
    # Clean up timeout marker
    rm -f "$WORK_DIR/.timeout"
...
echo "$(date '+%Y-%m-%d %H:%M:%S') - Starting Claude agent" >> "$RUN_LOG"
echo "  Model: $MODEL" >> "$RUN_LOG"
if [[ -n "$RESUME" ]]; then
    echo "  Resuming session: $RESUME" >> "$RUN_LOG"
fi
echo "  Max runtime: ${MAX_RUNTIME}s" >> "$RUN_LOG"
if [[ -n "$EXTENDED_THINKING" ]]; then
    echo "  Extended thinking: $EXTENDED_THINKING tokens" >> "$RUN_LOG"
fi
...
if [[ -n "$CENTRAL_LOG" ]]; then
    ...
    if [[ -x "$LOGGER_SCRIPT" ]]; then
        ...
        echo "$(date '+%Y-%m-%d %H:%M:%S') - Background logger started (PID: $LOGGER_PID)" >> "$RUN_LOG"
        echo "$(date '+%Y-%m-%d %H:%M:%S') - Central log: $CENTRAL_LOG" >> "$RUN_LOG"
    else
        echo "WARNING: Background logger not found at $LOGGER_SCRIPT" >&2
        echo "$(date '+%Y-%m-%d %H:%M:%S') - Background logger not found" >> "$RUN_LOG"
    fi
fi
...
(
    ...
    if kill -0 "$AGENT_PID" 2>/dev/null; then
        echo "$(date '+%Y-%m-%d %H:%M:%S') - Max runtime (${MAX_RUNTIME}s) exceeded, killing agent" >> "$RUN_LOG"
        kill -9 "$AGENT_PID" 2>/dev/null || true
        touch "$WORK_DIR/.timeout"
    fi
) &
WATCHDOG_PID=$!
...
# Check if agent was killed by runtime limit
if [[ -f "$WORK_DIR/.timeout" ]]; then
    rm -f "$WORK_DIR/.timeout"
    echo "ERROR: Maximum runtime (${MAX_RUNTIME}s) exceeded" >&2
    exit 2
fi

echo "$(date '+%Y-%m-%d %H:%M:%S') - Agent completed (exit code: $AGENT_EXIT_CODE)" >> "$RUN_LOG"
...
if [[ "$RESULT_IS_ERROR" == "true" ]]; then
    ...
    echo "$(date '+%Y-%m-%d %H:%M:%S') - Result is_error: true" >> "$RUN_LOG"
